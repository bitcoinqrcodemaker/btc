...
<!-- Mirrored from qrcode4bitcoin.com/ by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 26 Jan 2020 09:18:34 GMT -->
<head>
		<meta charset="utf-8">
		<meta name="description" content="Generate QR-Codes for Bitcoin securely using your browser.">
		<meta name="keywords" content="qrcode bitcoin uri secure javascript client-side download B logo cryptocurrency open source">
		<meta name="author" content="Aaron Breault, Marcus Triest, Len Schulwitz">
		<!--	To contact, you can e-mail last author's last name at gmail.com.
			All code below is open source AS IS use-at-your-own-risk code.
			We have built this site to be ad-free, cookie-free, spam-free, and scam-free site for the Bitcoin community.
			The code is written to be readible and understandable, the site's design is intended to be simple and easy to use,
			and the functionality of this site is intended to be impeccable for the Bitcoin community,
...
<body>
	<div class="header">
		...
		<div class="header-text"><label> Bitcoin QR</label></div>
	</div>
	<div class="page-container">

		<div class="input-container">
			<div class="tab">
				<button class="tablinks" onclick="selectTab(event, 'address_tab')" id="defaultOpen">Bitcoin QR Code</button>
			...
			<div id="address_tab" class="tabcontent">
				...
				<div class="uri-container">
					<div class="checkbox_container">
						...
						<div class="checkbox_label"><label>Bitcoin URI</label></div>
					</div>
					<br>
					<label>Amount</label>
...
<!--Hidden items below.-->
<img src="bitcoin_logo.svg" id="btc_logo_img_id">
<!--Start JS-->
...
var address_valid_state = "empty";
	...
	//alert("input_text: " + input_text );
		...
		if ( input_text != "" ) {
			...
			document.getElementById('qr_code_str').innerHTML = '';
	...
	//This function is needed to integrate BTC logo into QR image.
...
//		setTimeout(function(){							//We need to wait a few miliseconds for the image to load.
		qr_code_img.onload = function() {
			ctx.drawImage(qr_code_img, 0, 0);
			if ( custom_tab_open === false ){  			//Only draw the logo if it's a BTC QR code.
...
//		}, 5);	//This may be too short for slower systems.  I would use a callback, but it doesn't seem to work.
		}
	}
	...
	//This function builds the bitcoin URI string or sets the qr value to the (valid) bitcoin address or custom input.
			...
			if (addr_valid){
				if (uri_checked){
					...
					//Uri format specifications: https://github.com/bitcoin/bips/blob/master/bip-0021.mediawiki
						...
						qr_value = "bitcoin:" + "3NibWzUC7mFwkhdc6NsidRMuTokdwhYAxj";
					}
				...
				//If uri checkbox not checked, then the QR-code is just the address.
					...
					qr_value = "3NibWzUC7mFwkhdc6NsidRMuTokdwhYAxj";
					return qr_value;
				}
			}
